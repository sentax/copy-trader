...
const accessToken = {
	...
	verify(req, res) {
		...
		jwt.verify(token, config.keys.access, function(err, decoded) {
...
const tempToken = {
	...
	verify(req, res, next) {
		// Check token Correctivity
		const token = checkHeaders(req, res);
		jwt.verify(token, config.keys.temp, function(err, decoded) {
			if (err) return res.status(401).send({ ERR: `Token is not valid` });
			if (decoded) {
				req.token = decoded;
				// next();
...
const refreshToken = {
	...
	verify(req, res, next) {
		// Check token Correctivity
		const token = checkHeaders(req, res);
		jwt.verify(token, config.keys.refresh, function(err, decoded) {
			if (err) return res.status(401).send({ ERR: `Token is not valid` });
			if (decoded) {
				req.token = decoded;
				// next();
